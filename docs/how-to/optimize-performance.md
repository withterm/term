...
## Table of Contents
1. [Use the New Unified API](#use-the-new-unified-api)
2. [Optimize Statistical Constraints](#optimize-statistical-constraints)
3. [Batch Related Validations](#batch-related-validations)
4. [Memory Optimization](#memory-optimization)
5. [DataFusion Tuning](#datafusion-tuning)
6. [Large Dataset Considerations](#large-dataset-considerations)
7. [Performance Monitoring](#performance-monitoring)

## Use the New Unified API

The new unified constraint API (v0.2.0+) provides significant performance improvements:

### ❌ Avoid (Old API)
```rust
let check = Check::builder("stats")
    .has_min("price", Assertion::GreaterThan(0.0))
    .has_max("price", Assertion::LessThan(1000.0))
    .has_mean("price", Assertion::Between(50.0, 200.0))
    .build();
```
This executes 3 separate SQL queries.
...
let check = Check::builder("stats")
    .statistics(
        ...
        StatisticalOptions::new()
            ...
            .max(Assertion::LessThan(1000.0))
            .mean(Assertion::Between(50.0, 200.0))
    )?
    .build();
```
This executes 1 optimized query, resulting in **~27% performance improvement**.

## Optimize Statistical Constraints

### Combine Related Statistics
When validating multiple statistics on the same column, always use `StatisticalOptions`:

```rust
// ✅ GOOD: Single query for all statistics
.statistics(
    "response_time",
    StatisticalOptions::new()
...
// Set RUST_LOG=term_core=debug for detailed timing
```
...
- [ ] Using unified API methods (`completeness`, `statistics`, etc.)
...
Based on benchmarks with the new API:
