...
**Before:**
```rust
use term_core::constraints::UniquenessConstraint;
...
let unique_combo = UnifiedUniquenessConstraint::multiple(
    ...
    UniquenessOptions::new().null_handling(
        term_core::constraints::NullHandling::Exclude
    )
)?;
```
...
**Before:**
```rust
use term_core::constraints::{MinConstraint, MaxConstraint, MeanConstraint};
...
```rust
// Old
use term_core::constraints::{EmailConstraint, PatternConstraint, UrlConstraint};
...
[dependencies]
term-core = "=0.1.9"  # Last version before unified constraints
```
...
```toml
[dependencies]
term-core = { version = "0.2.0", features = ["legacy-constraints"] }
```
...
use term_core::constraints::EmailConstraint;
