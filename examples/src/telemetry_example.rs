...
async fn main() -> std::result::Result<(), Box<dyn Error>> {
    ...
    #[cfg(feature = "telemetry")]
    {
            ...
            .with_config(
                opentelemetry_sdk::trace::Config::default()
                    ...
                    .with_resource(Resource::new([
                        KeyValue::new("service.name", "term-validation-example"),
        ...
        let metrics_exporter =
            opentelemetry_sdk::testing::metrics::InMemoryMetricsExporter::default();
        ...
        if !exported_metrics.is_empty() {
            ...
            for (i, resource_metrics) in exported_metrics.iter().take(3).enumerate() {
                if let Some(scope_metrics) = resource_metrics.scope_metrics.first() {
                    if let Some(metric) = scope_metrics.metrics.first() {
