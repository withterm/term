...
use term_guard::constraints::{Assertion, SizeConstraint};
...
async fn main() -> std::result::Result<(), Box<dyn Error>> {
    ...
    #[cfg(feature = "telemetry")]
    {
        ...
        let provider = SdkTracerProvider::builder()
            .with_resource(
                Resource::builder()
                    .with_service_name("term-logging-example")
        ...
        let env_filter = EnvFilter::try_from_default_env()
            .unwrap_or_else(|_| EnvFilter::new("info,term_guard=debug"));
        ...
        println!("   - Log levels: info,term_guard=debug");
        println!();

        // Create Term telemetry configuration
        let telemetry = TermTelemetry::new(opentelemetry::global::BoxedTracer::new(Box::new(
            provider.tracer("term"),
        )))
    ...
    #[cfg(not(feature = "telemetry"))]
    {
        ...
        tracing_subscriber::fmt()
            .with_env_filter("info,term_guard=debug")
            .init();
